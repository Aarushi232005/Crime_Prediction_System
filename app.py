...
# Use joblib to load the model for better compatibility
model_path = os.path.join('Model', 'model.pkl')

# Ensure compatibility by checking for the correct model file
try:
    model = joblib.load(model_path)
except:
    model = pickle.load(open(model_path, 'rb'))  # Fallback to pickle
...
def predict_result():
    ...
    # Adjust population growth assuming 1% annual increase
    ...
    # Determine crime status based on the predicted crime rate
    if crime_rate <= 1:
